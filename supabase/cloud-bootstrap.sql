...
-- ============================================================
-- Per-org email delivery config (own SMTP server / Resend key). Secrets are
-- write-only via the RPCs; only service_role reads them to send.
...
-- ============================================================
-- Self-service org deletion with a 48h grace period (the stream gateway sweeps
-- and permanently deletes orgs past delete_after).
...
begin
  if not auth_is_admin() then raise exception 'admin only'; end if;
  update organizations set deletion_requested_at = now(), delete_after = v_after, deletion_requested_by = auth.uid() where id = auth_org();
  insert into audit_log(actor_id, actor_name, kind, context, detail)
    values (auth.uid(), (select full_name from profiles where id = auth.uid()), 'org_deletion_requested', auth_org()::text, jsonb_build_object('delete_after', v_after));
  return v_after;
end; $$;
...
begin
  if not auth_is_admin() then raise exception 'admin only'; end if;
  update organizations set deletion_requested_at = null, delete_after = null, deletion_requested_by = null where id = auth_org();
  insert into audit_log(actor_id, actor_name, kind, context, detail)
    values (auth.uid(), (select full_name from profiles where id = auth.uid()), 'org_deletion_cancelled', auth_org()::text, '{}'::jsonb);
end; $$;
...
begin
  ...
  return jsonb_build_object('scheduled', r.delete_after is not null, 'delete_after', r.delete_after, 'requested_at', r.deletion_requested_at, 'name', r.name);
end; $$;
