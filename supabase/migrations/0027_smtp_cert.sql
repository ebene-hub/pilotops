-- 0027_smtp_cert.sql
-- Allow an org to accept a mismatched/self-signed TLS certificate on its SMTP
-- server (common on shared cPanel hosting, where the cert is for *.web-hosting.com
-- rather than mail.yourdomain.com). The connection is still encrypted; only the
-- certificate NAME check is relaxed. Off by default.

alter table org_email_settings add column if not exists smtp_allow_invalid_cert boolean not null default false;

-- Re-create the read RPC to surface the new field.
...
-- Re-create the write RPC to persist the new field.
